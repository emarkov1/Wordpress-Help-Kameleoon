<?php
	...
	if (!defined('HIWEB_SEARCH_URL_CSS')) define('HIWEB_SEARCH_URL_CSS', WP_PLUGIN_URL . '/' . basename(HIWEB_SEARCH_DIR) . '/css');
	if (!defined('HIWEB_SEARCH_URL_JS')) define('HIWEB_SEARCH_URL_JS', WP_PLUGIN_URL . '/' . basename(HIWEB_SEARCH_DIR) . '/js');
	///Settings
	...
	if (!isset($hiweb_search_disallow_post_type)) {
		...
		$hiweb_search_disallow_post_type = array('attachment','revision','nav_menu_item');
	}
	...
	if (!isset($hiweb_search_meta_disallow_keys)) {
		...
		$hiweb_search_meta_disallow_keys = array('_edit_last', '_edit_lock');
	}
	if (!isset($hiweb_search_alternative_symbols)) {
		global $hiweb_search_alternative_symbols;
		$hiweb_search_alternative_symbols = array(
			'а' => 'о', 'е' => 'ё', 'и' => array('ы','а'),'ш' => 'щ','щ' => 'ш','ы' => 'и',
			'о' => 'а', 'ое' => 'ые'
		);
	}
	if (!isset($hiweb_search_trans_symbols)) {
		global $hiweb_search_trans_symbols;
		$hiweb_search_trans_symbols = array(
			'а' => 'a',   'б' => 'b',   'в' => 'v',
			'г' => 'g',   'д' => 'd',   'е' => 'e',
			'ё' => 'e',   'ж' => 'zh',  'з' => 'z',
			'и' => 'i',   'й' => 'y',   'к' => 'k',
	...
	if (!isset($hiweb_search_trans_symbols_back)) {
		...
		$hiweb_search_trans_symbols_back = array(
			'c' => 'к', 'y' => 'и', 'e' => 'е'
		);
	}